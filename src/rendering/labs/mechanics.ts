...
const physical: EffectRenderer = ({ c, node, age, world, state }) => {
    ...
    if (id === 'friction') {
        ...
        for (let x = 80; x < 920; x += 16)
            line(c, x, HEIGHT - 38, x - 8, HEIGHT - 29, FAINT);
    }
    if (!body)
        return;
    const speed = body.speed * 60 / PX_PER_M;
    if (id === 'impulse')
        text(c, age < p.t ? `Сила действует: ${age.toFixed(1)} / ${p.t} с` : 'Сила отключена. Движение по инерции.', 500, 575, 18, MUTED, 'center');
    else if (id === 'kinetic')
        text(c, `Текущая Eк = ${formatValue(.5 * body.mass * speed * speed)} Дж`, 190, 570, 18, INK, 'left', true);
    else if (id === 'momentum') {
        ...
        text(c, speed < .015 ? 'Тело остановилось' : `v = ${formatValue(speed)} м/с`, 160, 525, 21, INK, 'left', true);
    else
        text(c, `v = ${formatValue(speed)} м/с`, 135, 560, 18, INK, 'left', true);
    if (state.vectors) {
        const b = body.position;
        if (id === 'weight' || id === 'potential')
            arrow(c, b.x + 27, b.y, b.x + 27, b.y + Math.min(100, p.g * 5), INK);
        if (id === 'newton')
            arrow(c, b.x + 27, b.y, b.x + 27 + p.a * 10, b.y, INK);
        if (id === 'friction' && Math.abs(body.velocity.x) > .01)
            arrow(c, b.x, b.y - 28, b.x - Math.sign(body.velocity.x) * 60, b.y - 28, INK);
    }
};
const pressure: EffectRenderer = ({ c, node }) => {
    ...
    if (p.F > 0)
        arrow(c, 500, 134, 500, 218, INK, 2);
    text(c, `F = ${p.F} Н`, 553, 215, 23, INK, 'left', true);
    text(c, `S = ${p.S} м²`, 500, 574, 24, INK, 'center', true);
    if (p.F > 0)
        for (let x = 500 - width / 2 + 15; x < 500 + width / 2; x += 30)
            arrow(c, x, 457, x, 490 + deformation * .2, MUTED);
...
const fluid: EffectRenderer = ({ c, node, age }) => {
    ...
    for (let x = 270; x <= 730; x += 10)
        line(c, x, 270 + Math.sin(x * .05) * 2, x + 10, 270 + Math.sin((x + 10) * .05) * 2, '#c2c2c2');
    const target = density < rho ? 270 + side * (density / rho - .5) : 550 - side / 2, progress = 1 - Math.exp(-age * Math.sqrt(g / 9.8)), y = 365 + (target - 365) * progress;
    rect(c, 500 - side / 2, y - side / 2, side, side, PAPER, INK);
    text(c, 'm', 500, y + 8, 29, INK, 'center', true);
    const fraction = Math.min(1, Math.max(0, (y + side / 2 - 270) / side)), fa = rho * g * V * fraction;
    if (g > 0) {
    ...
    text(c, `FA сейчас ≈ ${formatValue(fa)} Н  ·  mg = ${formatValue(m * g)} Н`, 500, 633, 17, MUTED, 'center');
};
...
const blackhole: EffectRenderer = ({ c, node, world }) => {
    ...
    text(c, `Поглощено тел: ${world.absorbed.size}  ·  добавляйте тела кнопкой «+ Тело»`, 500, 631, 16, MUTED, 'center');
};
export const mechanicsEffects: Record<string, EffectRenderer> = { weight: physical, newton: physical, kinetic: physical, momentum: physical, potential: physical, impulse: physical, work: physical, hooke: physical, friction: physical, pressure, density: fluid, buoyancy: fluid, gravitation, blackhole };
