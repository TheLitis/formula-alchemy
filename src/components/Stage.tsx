...
export function Stage({ onReset, onHelp, onInspect }: { onReset: () => void; onHelp: () => void; onInspect: () => void }) {
    ...
    useEffect(() => {
        ...
        if (new URLSearchParams(location.search).has('qa') || (window as unknown as Record<string, unknown>).__ALCHEMY_QA__ === true) (window as unknown as Record<string, unknown>).__alchemyTest = { store, runtime, renderer: draw, audio };
