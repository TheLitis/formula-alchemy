...
export function App() {
    ...
    const toggleMusic = () => {
        ...
        // Start the media element synchronously inside the user gesture.
        store.patch({ music: enabled });
    };
    return <><div className="app-shell" id="app-shell"><header className="app-header"><a className="wordmark" href="#" onClick={e => { e.preventDefault(); store.patch({ lab: 'sandbox', activeId: null, selectedId: null }); }} aria-label="Formula Alchemy — на холст"><span className="brand-mark">ƒ</span><span>Formula <em>Alchemy</em><small>ФИЗИКА В ВАШИХ РУКАХ</small></span></a>
      ...
      <div className="header-tools"><button className={`icon-button music-toggle ${state.music ? 'active' : ''}`} title={state.music ? 'Выключить музыку: Elektronomia — Sky High' : 'Включить музыку: Elektronomia — Sky High'} aria-label={state.music ? 'Выключить музыку: Elektronomia — Sky High' : 'Включить музыку: Elektronomia — Sky High'} aria-pressed={state.music} onClick={toggleMusic}><Icon name="music" size={17}/></button><button className="icon-button sound-toggle" aria-label={state.sound ? 'Выключить звуки' : 'Включить звуки'} title={state.sound ? 'Выключить звуки' : 'Включить звуки'} aria-pressed={state.sound} onClick={() => { const enabled = !store.getState().sound; store.patch({ sound: enabled }); if (enabled) void audio.unlock().then(() => audio.click()).catch(() => store.notify('Аудио недоступно в этом браузере.', 'error')); }}><Icon name={state.sound ? 'volume' : 'muted'} size={17}/></button><button className="button dark header-save" aria-label="Сохранить" onClick={() => setModal('save')}><Icon name="save" size={16}/><span>Сохранить</span></button></div>
    </header>
