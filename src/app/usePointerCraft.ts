...
export function usePointerCraft(board: RefObject<HTMLDivElement | null>, renderer: RefObject<CanvasRenderer | null>) {
  ...
  useEffect(() => {
    ...
    const clean = () => {
      if (drag) drag.source.style.opacity = '';
      ghost?.remove();
      ghost = null;
      drag = null;
      clearHighlights();
    ...
    const burst = (x: number, y: number, symbol = '✦') => {
      const parent = board.current;
      if (!parent) return;
      const r = parent.getBoundingClientRect();
      const el = document.createElement('div');
      el.className = 'combine-burst';
    ...
    const down = (event: PointerEvent) => {
      if (event.button !== 0 || event.isPrimary === false || document.querySelector('[aria-modal="true"]')) return;
    ...
    const hoverTarget = (x: number, y: number) => [...document.querySelectorAll<HTMLElement>('[data-node]')].find(el => {
      if (drag?.kind === 'node' && el.dataset.node === drag.id) return false;
    ...
    const move = (event: PointerEvent) => {
      ...
      if (target && drag.moved) {
        ...
        if (b) target.classList.add(candidates([...a, ...b.parts]).length ? 'drop-good' : 'drop-bad');
      }
    };
    ...
    const up = (event: PointerEvent) => {
      ...
      const trash = [...document.querySelectorAll<HTMLElement>('[data-trash]')].some(el => {
      ...
      if (bounds && renderer.current && event.clientX >= bounds.left && event.clientX <= bounds.right && event.clientY >= bounds.top && event.clientY <= bounds.bottom) {
        ...
        let merged = false;
        if (target) {
          merged = current.kind === 'symbol' ? store.dropSymbol(current.id, target.dataset.node!) : store.combine(current.id, target.dataset.node!);
          if (merged) {
            burst(event.clientX, event.clientY, '✦');
            audio.discovery();
          }
